        ...
        >
          ...
          <Formik
            ...
            onSubmit={async (values, actions) => {
              ...
              try {
                const createEnq = await API.graphql<GraphQLQuery<CreateEnquiryInput>>({
                  query: `
                mutation MyMutation {
                  createEnquiry(input: {number_of_people: ${Number(values.numberOfPeople)}, name: "${values.name}", phone_number: "${values.phone}" vacation_type: "${values.shootingCategory}", email: "${values.email}", destination_name: "${values.destinationName}"}) {
                    email
                    id
                    name
                    number_of_people
            ...
            {props => (
              <Form>
                <Stack spacing={4} fontFamily="'Roboto'" fontSize="16px">
                  ...
                  <Field name="numberOfPeople" validate={validateNumberOfPeople}>
                    {({ field, form }) => (
                      <FormControl
                        isInvalid={form.errors.numberOfPeople && form.touched.numberOfPeople}
                      >
                        <Input {...field} id="numberOfPeople" placeholder="No. Of People" />
